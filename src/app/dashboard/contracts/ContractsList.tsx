    ...
    <>
      ...
      <div className="mt-4 rounded-xl bg-white ring-1 ring-zinc-200 overflow-hidden">
        ...
        <div className="hidden md:block">
          <div className="grid grid-cols-[140px_110px_1fr_180px_120px_120px_24px] gap-3 px-5 py-2.5 text-[11px] uppercase tracking-wider text-zinc-400 border-b border-zinc-100">
          ...
          {filtered.map((c) => (
            <Link
              ...
              className="grid grid-cols-[140px_110px_1fr_180px_120px_120px_24px] gap-3 items-center px-5 py-3 border-b border-zinc-50 last:border-0 text-sm hover:bg-zinc-50"
            >
              <span className="font-mono text-xs">{c.contract_nr}</span>
              <span className="font-mono font-semibold">{c.plate}</span>
              <span className="text-zinc-900 truncate">{c.renter_name}</span>
              <span className="text-zinc-500 text-xs truncate">{c.renter_email || "—"}</span>
              <span className="text-zinc-700 tabular-nums text-xs">
                {fmtDate(c.pickup_date)}
                <br />
                <span className="text-zinc-400">→ {fmtDate(c.actual_return_date || c.return_date)}</span>
              </span>
            ...
            >
              <div className="flex-1 min-w-0 space-y-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="ml-auto font-mono text-[11px] text-zinc-500">{c.contract_nr}</span>
                </div>
                <div className="text-sm text-zinc-900 truncate">{c.renter_name}</div>
        ...
        {filtered.length === 0 && (
          <div className="px-5 py-12 text-center text-sm text-zinc-500">
            <FileSignature size={28} className="mx-auto text-zinc-300" />
