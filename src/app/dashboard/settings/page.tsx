...
const SAFE_COLUMNS =
  "id, name, street, zip, city, phone, email, tax_number, processing_fee, iban, bic, account_holder, kleinunternehmer, slug, inbound_email, sender_name, sender_email, lexoffice_enabled, echoes_account_id, echoes_enabled, credit_provider, credit_api_url, email_domain, email_domain_id, email_domain_status, email_dns_records, contract_email_subject, contract_email_body, rental_terms, logo_path, brand_color, landlord_signature_name, created_at";
...
export default async function SettingsPage() {
  ...
  const { data } = await supabase
    .from("organizations")
    .select(
      `${SAFE_COLUMNS}, lexoffice_api_key, echoes_api_key, credit_api_key, shopify_shop_domain, shopify_admin_token, shopify_webhook_token, landlord_signature_data`
    )
    .single();
  ...
  const creditHasKey =
    typeof credit_api_key === "string" && credit_api_key.length > 0;
