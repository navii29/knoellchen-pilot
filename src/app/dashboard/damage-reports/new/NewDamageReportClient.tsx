  ...
  return (
    <>
      ...
      <PageHeader
        ...
        description="Datum, Ort, Beschreibung und Fotos. Optional: Vertrag/Fahrzeug zuordnen + Gegnerdaten."
      />
        ...
        <Panel>
          <Section title="Zuordnung">
            <Field label="Vertrag (optional)">
              <select value={data.contract_id} onChange={(e) => pickContract(e.target.value)} className="field">
            ...
            <Field label="Fahrzeug">
              <select value={data.vehicle_id} onChange={set("vehicle_id")} className="field">
            ...
            <div className="sm:col-span-2">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {stagedPreviews.map((src, i) => (
                  <div
                    key={i}
                    className="relative aspect-square rounded-panel overflow-hidden bg-canvas border border-hairline"
                  >
                    ...
                    <button
                      ...
                      className="absolute top-1.5 right-1.5 w-6 h-6 rounded-full bg-white/90 backdrop-blur text-ink-soft hover:text-red-600 flex items-center justify-center shadow"
                    >
                      <Trash2 size={11} />
                    </button>
                  </div>
...
const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
  <div>
    <div className="kicker text-ink-muted mb-3">{title}</div>
