...
const Step4Sign = ({
  ...
  return (
    <StepCard
      ...
      onBack={onBack}
    >
      <div className="bg-paper border border-hairline rounded-card p-4 mb-4 space-y-1.5">
      ...
      {alreadySigned ? (
        <div className="bg-canvas border border-hairline rounded-panel px-4 py-3 text-[13px] text-ink-soft flex items-center gap-2 mb-4">
          <Check size={14} /> Bereits unterschrieben — du kannst direkt zum Abschluss.
        </div>
      ) : (
        <>
          <div className="bg-paper border border-hairline rounded-card p-4 mb-3">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-1.5 kicker text-ink-muted">
                <FileSignature size={11} /> Deine Unterschrift
              </div>
              <button
                type="button"
                onClick={() => {
      ...
      {error && (
        <div className="flex items-center gap-2 text-[13px] rounded-input px-3 py-2 bg-rose-50 border border-rose-200 text-rose-700 mb-3">
      ...
      <Button
        ...
        size="lg"
        onClick={submit}
        disabled={submitting || (!alreadySigned && (!hasInk || !accepted))}
        className="w-full"
      >
        {submitting ? (
...
const StepDone = ({ onFinish }: { onFinish: () => void }) => (
  <div className="bg-paper border border-hairline rounded-card shadow-panel p-7 text-center">
    ...
    <Button type="button" variant="signal" size="lg" onClick={onFinish} className="mt-5">
      Zurück zum Vertrag
    </Button>
  </div>
);
