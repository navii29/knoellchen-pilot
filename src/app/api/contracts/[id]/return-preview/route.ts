...
export const POST = async (req: Request, { params }: { params: { id: string } }) => {
  ...
  const { data: contract } = await admin
    .from("contracts")
    .select("pickup_date, return_date, original_return_date, daily_rate, weekly_rate, monthly_rate, km_pickup, km_limit, plate")
    .eq("id", params.id)
  ...
  if (contract.plate) {
    const { data: v } = await admin
      .from("vehicles")
      .select("extra_km_price, inclusive_km_month, daily_rate, weekly_rate, monthly_rate, vehicle_type, manufacturer, model")
      .eq("org_id", auth.org_id)
  ...
  const dailyRate = resolveEffectiveDailyRate({
    ...
    vehicleMonthlyRate,
    contractWeeklyRate: contract.weekly_rate as number | null,
    vehicleWeeklyRate,
  });
