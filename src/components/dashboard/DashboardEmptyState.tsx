...
const Action = ({
  ...
  <Link
    href={href}
    className="group flex items-start gap-3.5 rounded-2xl bg-white ring-1 ring-zinc-200 p-5 hover:ring-zinc-300 hover:shadow-sm transition"
  >
    <div className="w-10 h-10 rounded-xl bg-indigo-50 ring-1 ring-indigo-100 text-indigo-700 flex items-center justify-center shrink-0">
      <Icon size={18} strokeWidth={1.75} />
    </div>
    <div className="min-w-0">
      <div className="flex items-center gap-1 text-[14.5px] font-medium text-zinc-900">
        {title}
        <ArrowRight size={14} className="text-zinc-400 group-hover:translate-x-0.5 transition-transform" />
      </div>
      <div className="text-[13px] text-zinc-500 mt-0.5 leading-snug">{desc}</div>
    </div>
  </Link>
);
  ...
  return (
    <div className="rounded-3xl bg-white ring-1 ring-black/[0.05] p-8 sm:p-12">
      <div className="max-w-2xl">
        <div className="inline-flex items-center gap-2 px-3 h-7 rounded-full bg-indigo-50 ring-1 ring-indigo-100 text-[12px] text-indigo-700 font-medium mb-5">
          <Sparkles size={13} />
          Erste Schritte
        </div>
        <h2 className="font-display font-medium text-[24px] sm:text-[30px] leading-[1.1] tracking-[-0.02em] text-zinc-900">
          Willkommen — richten wir Ihre Vermietung ein.
        </h2>
        <p className="text-[14.5px] text-zinc-500 mt-3 leading-relaxed">
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3 border-t border-zinc-100 pt-6">
        <button
          onClick={loadDemo}
          disabled={loading}
          className="inline-flex items-center gap-2 px-4 h-10 rounded-full bg-zinc-900 text-white text-[14px] font-medium hover:bg-zinc-800 transition disabled:opacity-60"
        >
