...
<body style="margin:0;padding:0;background:#fafafa;font-family:'DM Sans',-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,sans-serif;color:#18181b;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#fafafa;padding:32px 16px;">
    <tr><td align="center">
      <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="max-width:600px;background:#ffffff;border-radius:14px;border:1px solid #e4e4e7;overflow:hidden;">
        <tr><td style="padding:32px 36px;">${inner}</td></tr>
        <tr><td style="padding:20px 36px;border-top:1px solid #f4f4f5;background:#fafafa;color:#71717a;font-size:12px;line-height:1.55;">${footerHtml}</td></tr>
      </table>
    </td></tr>
  </table>
</body></html>`;
...
<strong style="color:#3f3f46;">${escape(org.name)}</strong>${org.street ? "<br>" + escape(org.street) : ""}${
  org.zip || org.city ? "<br>" + escape([org.zip, org.city].filter(Boolean).join(" ")) : ""
}${org.phone ? '<br><span style="color:#a1a1aa;">Tel.</span> ' + escape(org.phone) : ""}${
  org.email ? '<br><span style="color:#a1a1aa;">E-Mail</span> ' + escape(org.email) : ""
}${org.tax_number ? '<br><span style="color:#a1a1aa;">USt-IdNr.</span> ' + escape(org.tax_number) : ""}
<div style="margin-top:14px;color:#a1a1aa;font-size:11px;">Versendet via Knöllchen-Pilot · automatisierte Strafzettel-Bearbeitung</div>`;
...
export const renterEmail = (
  ...
  const html = wrap(
    subject,
    `
    <div style="font-size:13px;color:#a1a1aa;letter-spacing:.05em;text-transform:uppercase;font-weight:600;">${escape(
      ticket.ticket_nr
    )}${contract?.contract_nr ? " · Vertrag " + escape(contract.contract_nr) : ""}</div>
    <h1 style="margin:8px 0 24px;font-size:22px;line-height:1.3;font-weight:700;color:#18181b;">Weiterbelastung Ordnungswidrigkeit ${escape(
      ticket.plate
    )}</h1>
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border:1px solid #e4e4e7;border-radius:10px;margin:16px 0 24px;">
      ${[
        ...
        .map(
          ([k, v]) =>
            `<tr><td style="padding:9px 16px;border-bottom:1px solid #f4f4f5;color:#71717a;font-size:13px;width:140px;">${escape(
              k
            )}</td><td style="padding:9px 16px;border-bottom:1px solid #f4f4f5;font-size:13px;color:#18181b;">${escape(
              v
            )}</td></tr>`
        )
        .join("")}
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#fafafa;border-radius:10px;margin:0 0 24px;">
      <tr><td style="padding:14px 18px;color:#71717a;font-size:13px;">Bußgeld</td><td style="padding:14px 18px;text-align:right;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:14px;">${fmtEur(
        ticket.fine_amount
      )}</td></tr>
      <tr><td style="padding:14px 18px;color:#71717a;font-size:13px;border-top:1px solid #f4f4f5;">Bearbeitungsgebühr</td><td style="padding:14px 18px;text-align:right;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:14px;border-top:1px solid #f4f4f5;">${fmtEur(
        Number(ticket.processing_fee)
      )}</td></tr>
      <tr><td style="padding:14px 18px;font-weight:600;border-top:1px solid #e4e4e7;">Gesamtbetrag</td><td style="padding:14px 18px;text-align:right;font-family:'JetBrains Mono',ui-monospace,monospace;font-weight:700;font-size:16px;border-top:1px solid #e4e4e7;color:#4f46e5;">${fmtEur(
        total
      )}</td></tr>
    </table>
    ...
    <p style="margin:0 0 24px;line-height:1.65;color:#71717a;font-size:13px;">Im Anhang finden Sie unser ausführliches Anschreiben sowie die Rechnung als PDF.</p>
...
export const authorityEmail = (
  ...
  const html = wrap(
    subject,
    `
    <div style="font-size:13px;color:#a1a1aa;letter-spacing:.05em;text-transform:uppercase;font-weight:600;">Fahrerermittlung</div>
    <h1 style="margin:8px 0 24px;font-size:22px;line-height:1.3;font-weight:700;color:#18181b;">Aktenzeichen ${escape(
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#fafafa;border-radius:10px;margin:0 0 24px;">
      <tr><td style="padding:11px 18px;color:#71717a;font-size:13px;width:140px;">Name</td><td style="padding:11px 18px;font-weight:600;">${escape(
        renterName
      )}</td></tr>
      <tr><td style="padding:11px 18px;color:#71717a;font-size:13px;border-top:1px solid #f4f4f5;">Anschrift</td><td style="padding:11px 18px;border-top:1px solid #f4f4f5;">${escape(
        renterAddress
      )}</td></tr>
      <tr><td style="padding:11px 18px;color:#71717a;font-size:13px;border-top:1px solid #f4f4f5;">Geburtsdatum</td><td style="padding:11px 18px;border-top:1px solid #f4f4f5;font-family:'JetBrains Mono',ui-monospace,monospace;">${escape(
        renterBirthday
      )}</td></tr>
      ${contract?.contract_nr ? `<tr><td style="padding:11px 18px;color:#71717a;font-size:13px;border-top:1px solid #f4f4f5;">Mietvertrag</td><td style="padding:11px 18px;border-top:1px solid #f4f4f5;font-family:'JetBrains Mono',ui-monospace,monospace;">${escape(contract.contract_nr)}</td></tr>` : ""}
      ${contract ? `<tr><td style="padding:11px 18px;color:#71717a;font-size:13px;border-top:1px solid #f4f4f5;">Mietzeitraum</td><td style="padding:11px 18px;border-top:1px solid #f4f4f5;">${fmtDate(contract.pickup_date)} – ${fmtDate(contract.actual_return_date || contract.return_date)}</td></tr>` : ""}
    </table>
